...
package body data is
   ...
   procedure Setdata (taskup:in out task_set) is
      data: integer:=0;
   begin
      for i in taskup'range loop
         Put("Introduzca el T_Computo de la tarea ");Put(i,1);Put(": ");
         Get(data);
         taskup(i).wcet:=data;
         Put("Introduzca el Plazo de la tarea ");Put(i,1);Put(": ");
         Get(data);
         taskup(i).deadline:=data;
         Put("Introduzca el Periodo de la tarea ");Put(i,1);Put(": ");
         Get(data);
         taskup(i).period:=data;
      end loop;
   end Setdata;
